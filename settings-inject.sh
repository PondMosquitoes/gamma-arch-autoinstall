#!/usr/bin/env bash
# settings-inject.sh — restore STALKER GAMMA settings from settings/
set -euo pipefail
...
# ── user.ltx — graphics settings + keybinds ──────────────────────────────────
...
# ── MCM settings — inject into whichever MCM values mod is present ────────────
...
if [[ -z "$MCM_DEST" ]]; then
    # mod folder exists but file not yet created — derive path from folder
    for candidate_dir in "$STALKER/GAMMA/mods/"*"MCM values"*/; do
...
if [[ -n "$MCM_DEST" ]] && [[ -f "$SETTINGS/mcm/axr_options.ltx" ]]; then
    ...
    warn "MCM values mod not found under GAMMA/mods/ — skipping"
fi

# ── MO2 modlists — all profiles ───────────────────────────────────────────────
...
for profile_settings in "$SETTINGS/profiles"/*/; do
    ...
    dest_dir="$STALKER/GAMMA/profiles/$profile_name"
    if [[ ! -d "$dest_dir" ]]; then
